...
export const handlers = [
  ...
  rest.get('/msw/no-content-length/1', async (_, res, ctx) => {
    const imageBuffer = await fetch(IMAGES[1].replace(PREFIX, '')).then(r =>
      r.arrayBuffer()
    )
    return res(ctx.body(imageBuffer))
  }),
  rest.get('/msw/no-content-length/2', async (_, res, ctx) => {
    const imageBuffer = await fetch(IMAGES[6].replace(PREFIX, '')).then(r =>
      r.arrayBuffer()
    )
    return res(ctx.body(imageBuffer))
  }),
  rest.get('/msw/no-content-length/3', async (_, res, ctx) => {
    const imageBuffer = await fetch(IMAGES[7].replace(PREFIX, '')).then(r =>
      r.arrayBuffer()
    )
    return res(ctx.body(imageBuffer))
  }),
  rest.get('/msw/no-content-length/4', async (_, res, ctx) => {
    const imageBuffer = await fetch(IMAGES[8].replace(PREFIX, '')).then(r =>
      r.arrayBuffer()
    )
    return res(ctx.body(imageBuffer))
  }),
