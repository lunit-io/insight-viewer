...
import type { Annotation, AnnotationBase } from '../../types'
    ...
    setAnnotations(() =>
      initialAnnotation
        ? initialAnnotation.map<Annotation>((addedAnnotation, i) => {
            const annotationPoints = getAnnotationPoints(addedAnnotation)

            return {
              ...addedAnnotation,
              id: addedAnnotation.id ?? nextId + i,
              labelPosition: addedAnnotation.labelPosition ?? polylabel([annotationPoints], 1),
            } as Annotation
          })
        : []
    )
